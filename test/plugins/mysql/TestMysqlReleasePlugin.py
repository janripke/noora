...
from org.noora.plugin.mysql.release.ReleasePlugin import ReleasePlugin
...
class TestBase(unittest.TestCase): 
  ...
  def testUpdatePass(self):    
    ...
    print(releasePlugin.getRevision())
    options = releasePlugin.getOptions(properties)

    arguments = ['-n=example', '-v=1.0.0']
    parser = Parser()
    commandLine = parser.parse(options,arguments)
    parser.checkRequiredOptions()
    parser.checkRequiredArguments()
    releasePlugin.execute(commandLine, properties)
