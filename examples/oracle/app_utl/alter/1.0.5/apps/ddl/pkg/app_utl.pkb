  ...
  */
  ...
  begin
    ...
    loop
      ...
      -- when the delimeter is null the returned position is null
      -- in this case the position will be 0.
  ...
  procedure sync_sequence
    (p_sequence in varchar
    ,p_table in varchar2
    ,p_pk_field in varchar2) is
    
    pragma autonomous_transaction;    
    
    l_pk_id number;
    l_seq_value number := 0;
  begin
    execute immediate 'select nvl(max(' || p_pk_field || '), 0) from ' || p_table into l_pk_id;
    while l_seq_value < l_pk_id loop
      execute immediate 'select ' || p_sequence || '.nextval from dual' into l_seq_value;
    end loop;
  end sync_sequence;
  ...
  function get_line_terminator
    ...
    l_prc_name varchar2(50):='get_line_terminator';
  ...
  begin
    ...
    logger.debug(p_job_name,M_PACKAGE_NAME,l_prc_name,instr(l_buffer,chr(13)));
    logger.debug(p_job_name,M_PACKAGE_NAME,l_prc_name,l_buffer);
    ...
    -- TODO:the one line scenario
    -- TODO:the empty file scenario
  ...
  function get_optionally_enclosed
    (p_optionally_enclosed_yn in varchar2) return varchar2 is
  ...
  -- returns the fields_terminated_by statement.
  -- this statementment is used by the create_external_table statement.
  -- remarks : when no field_delimiter is given an empty string is returned.  
  ...
  begin
    if p_field_delimiter is not null then
      l_result := ' fields terminated by ' || M_QUOTE || p_field_delimiter || M_QUOTE ||
  ...
  procedure create_external_table
    ...
    ,p_field_list             in t_field_list
    ,p_characterset           in varchar2 default null) is
  ...
  l_characterset varchar2(50);
  begin
    if p_characterset is not null then
      l_characterset := ' CHARACTERSET ''' || p_characterset || ''' ';
    end if;
                  ...
                  organization external(
                    ...
                    access parameters ( ' ||
                      ...
                      ' BADFILE ' || M_QUOTE || l_filename || '.bad' || M_QUOTE || '
                       ...
                       get_fields_terminated_by(p_field_delimiter, p_optionally_enclosed_yn) || 
  ...
  procedure gather_stats(p_job_name in varchar2, p_table_name in varchar2) is
    cursor c_index (b_table_name varchar2) is
      select index_name
      from user_indexes i
      where i.table_name = upper(b_table_name)
      and i.table_owner = upper(user);
    l_prc_name varchar2(30) := 'gather_stats';
  begin
    logger.debug(p_job_name,M_PACKAGE_NAME,l_prc_name,'Gathering table stats for :' || p_table_name);
    dbms_stats.gather_table_stats(ownname => user, tabname => p_table_name);
    for c in c_index(b_table_name=>p_table_name) loop
      logger.debug(p_job_name,M_PACKAGE_NAME,l_prc_name,'Gathering index stats for :' || c.index_name);
      dbms_stats.gather_index_stats(ownname => user, indname => c.index_name);
    end loop;  
    logger.debug(p_job_name,M_PACKAGE_NAME,l_prc_name,'end');
  end gather_stats;
  
  
  -- sets a status that the specified process is running. This will go the session variables Module and Action in v$session.
  ...
  -- checks if a process is running. Must have been set by procedure set_process_running
  ...
  return boolean is
    ...
    cursor c_session is
      select 1 
      from v$session s 
      where upper(s.MODULE) = upper(p_module_name)
      and upper(s.ACTION) = upper(p_action_name)
      and s.STATUS = 'ACTIVE'
