...
def version_folder_not_present(version):
  version_folder=ALTER_DIR+os.sep+version
  if utils.dir_is_not_present(version_folder):
    usage()
    print
...
def previous_version_not_none(previous_version):
  if previous_version==None:
    usage()
    print
    print "no previous version was found"
    exit(1)    
...
def check_version(oracle_sid,oracle_user,oracle_passwd,previous_version,version_select_statement):
   oracle_script=NOORA_DIR+os.sep+'checkversion.sql'
...
def check_environment(oracle_sid,oracle_user,oracle_passwd,environment,environment_select_statement):
   oracle_script=NOORA_DIR+os.sep+'checkenvironment.sql'
...
if __name__ == "__main__":
  ...
  version_folder_not_present(version)
  
  previous_version=get_previous_version(version)
  previous_version_not_none(previous_version)

  oracle_user=get_oracle_user(VERSION_SCHEMA,environment)
  oracle_passwd=get_oracle_passwd(VERSION_SCHEMA,environment)
