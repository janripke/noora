create or replace package body register is
  ...
  procedure upd_file_by_job_name
    (p_file     in files%rowtype) is
  ...
  begin
    ...
    update files
       set filename           = p_file.filename
          ...
          ,records_ignored    = nvl(p_file.records_ignored,0)
          ,external_id        = p_file.external_id
     where job_name = p_file.job_name
	 log errors into files_err reject limit 0;
  ...
  begin
	...
	if l_file.job_name is not null then
	  upd_file_by_job_name
	    (p_file     => l_file);
	elsif l_file.id is not null then
	  upd_file
        (p_file     => l_file);
	else
	  upd_file
