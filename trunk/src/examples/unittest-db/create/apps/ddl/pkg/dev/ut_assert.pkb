create or replace package body UT_ASSERT as
  ...
  function bool_to_char(p_boolean in boolean) return varchar2 is

    l_result   varchar2(10):=chr(0);
  ...
  begin
    ...
    then
      raise_application_error(-20001, p_message || ' ' || nvl(a, 'null') || ' <> ' || nvl(b, 'null'));
    end if;
  end;
  ...
  begin
    ...
    if nvl(a, chr(0)) = nvl(b, chr(0))
    then
      raise_application_error(-20002, p_message || ' ' || nvl(a, 'null') || ' <> ' || nvl(b, 'null'));
    end if;
  end;
  ...
  procedure throws(p_message     in varchar2
                  ,a             in varchar2
                  ,b             in varchar2) is
  ...
  exception
    ...
    when no_exception then
      raise_application_error(-20003, p_message || ' ' || 'null' || ' <> ' || nvl(b, 'null'));
    ...
    when others then
      ...
      then
        ...
        raise_application_error(-20003, p_message || ' ' || nvl(l_sqlerrm, 'null') || ' <> ' || nvl(b, 'null'));
      end if;
