...
from org.noora.plugin.postgresql.update.UnknownVersionException import UnknownVersionException
from org.noora.plugin.postgresql.update.InvalidEnvironmentException import InvalidEnvironmentException
from org.noora.plugin.postgresql.update.InvalidVersionException import InvalidVersionException
...
class GeneratePlugin(Plugin):
  ...
  def getOptions(self, properties):
    ...
    options.addOption("-h", "--host", False, False, "hostname of the postgres database.")
    options.addOption("-p", "--port", False, False, "port for the postgres database.")
    options.addOption("-d", "--database", False, False,  "database in the postgresql-server.")
    options.addOption("-u", "--username", False, False,  "username in the postgresql-server.")
    #options.addOption("-p", "--password", False, False,  "password in the postgresql-server.")
  ...
  def checkEnvironment(self, connector, executor, environment, properties):
    ...
    script = File(pluginFolder+os.sep+'postgresql'+os.sep+'update'+os.sep+'checkenvironment.sql')
  ...
  def checkVersion(self, connector, executor, version, properties):
    ...
    script = File(pluginFolder+os.sep+'postgresql'+os.sep+'update'+os.sep+'checkversion.sql')
    executor.setScript(script)      
    connector.execute(executor, properties)
    if "(Code 1329)" in connector.getProcessorResult().getResult():
  ...
  def execute(self, commandLine, properties):
    ...
    if configFile.exists()==False:      
      ...
      #create project.conf
      templateFolder = Path.path(properties.getPropertyValue('plugin.dir'),'postgresql','generate','templates')
      templateFile = File(Path.path(properties.getPropertyValue('plugin.dir'),'postgresql','generate','templates',properties.getPropertyValue('project.file')))
